...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = (from emp in Emps where emp.Job == "Frontend programmer" && emp.Salary > 1000 orderby emp.Ename descending select emp).ToList();
            /*
        ...
        public static IEnumerable<Emp> Task4()
        {
            ...
            //var result = Emps.Where(emp => emp.Salary == maxSal).ToList();
        ...
        {
            IEnumerable<object> result = Depts.Select(dept => new
            {
                name = dept.Dname,
                numOfEmployees = Emps.Count(emp => emp.Deptno == dept.Deptno)
            }).Where(emp => emp.numOfEmployees > 0).ToList(); 
        ...
        {
            int result = CustomExtensionMethods.findNumber(arr);
            Console.WriteLine(result);
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            ...
            .Where(emp => emp.LiczbaPracownikow == 5 || emp.LiczbaPracownikow == 0)
            .OrderByDescending(dept => dept.Dzial)
            .Select(dept => dept.Dzial);
    ...
    public static class CustomExtensionMethods
    {
        ...
        public static IEnumerable<Emp> Task12Method(this IEnumerable<Emp> employess)
        {
            var result = employess.Where(emp => emp.Mgr != null).Select(emp => emp.Mgr).Distinct().ToList();
            return result;
        }

        public static int findNumber(int[] arr) {
            //firstordefult - zwraca pierwszy element albo domyslny jak nie ma pierwszego
            var i = arr.GroupBy(i => i).Where(m => m.Count()%2==1).Select(i => i.Key).FirstOrDefault();
            return i;
        }
